...
// Retrieve the brand photo from the session
$brandphoto=$_SESSION['photo'];
// Check if the form is submitted
if(isset($_POST['submit'])){
    $message=$_POST['message'];
    $recevoir= $_GET['influencerusername'];
    $envoyer=$_SESSION['username'];
        // Check if the message is not empty
    if(!empty($message)){
      // Insert the message into the database
      $stmt = $p->insertmessage($envoyer, $recevoir,$message);
      header("Location: ".$_SERVER['PHP_SELF']."?influencerusername=".$_GET['influencerusername']);
      exit();
        } else{
            echo "Oops! Something went wrong. Please try again later.";
        }
    }  
    // Get the influencer ID from the URL parameter
$inlfluencerid=$_GET["influencerusername"];
// Get influencer details from the database
$results=$p->getInfluencer($inlfluencerid);
...
$email=$row['email'];}
// Get brand details from the session
$idb=$_SESSION['id'];
$brandusername=$_SESSION['username'];
...
<head>
  ...
  <link rel="stylesheet" href="brandcss/contacter.css" />
...
<body>
  ...
  <div class="container">
    ...
    <div class="chat">
        ...
        <div class="profile">
          <div class="left">
            <?php
              ...
              margin-bottom: 5px; " src="../influencerphp/influencerimg/'.$influencerphoto.'">
      ...
      <div class="chat-box">
        <?php
               $results= $p->selectmessage($brandusername, $inlfluencerid);
                if($results){
                    foreach($results as $row){
                            $date=$row['date'];
                            $message=$row['message'];
                            $envoyerX=$row['envoyer'];
                            if($envoyerX==$brandusername){
                                echo '
                                <div class="chat-r">
                                    ...
                                    margin-bottom: 5px; margin-top: 30px;  " src="brandimg/'.$brandphoto.'">
                                </div>';
                                ...
                                margin-bottom: 0px; margin-top: 27px; margin-right: 7px; " src="../influencerphp/influencerimg/'.$photo.'">
                                    <div class="mess">
                                    <p>'.$message.'</p>
                                    <div class="check">
                                        <span>'.$date.'</span>
    ...
    <?php
              ...
              <h2 style="  margin-top : 15px; font-size:40px;">'.$inlfluencerid.'</h2>';
